...
class DDPTrainer:
    def __init__(self, generator, discriminator, train_data, optimizer_g, optimizer_d, criterion, gpu_id,
                 use_wandb=False, config=None):
        self.gpu_id = gpu_id
        self.generator = DDP(generator.to(gpu_id), device_ids=[gpu_id])
        self.discriminator = DDP(discriminator.to(gpu_id), device_ids=[gpu_id])
        self.train_data = train_data
        self.optimizer_g = optimizer_g
        self.optimizer_d = optimizer_d
        self.criterion = criterion
        ...
        if use_wandb and self.gpu_id == 0:
            wandb.init(project="Pytorch Distributed Study",
                       config=config)
            wandb.run.name = "DDP_" + str(datetime.now())

    def _save_checkpoint(self):
        ckp = self.generator.module.state_dict()
        PATH = "./saved/checkpoint.pt"
        torch.save(ckp, PATH)
    ...
    def _run_epoch(self, epoch):
        ...
        for data in self.train_data:
            ...
            # Train with all-fake batch
            # Generate batch of latent vectors
            noise = torch.randn(b_size, self.generator.module.nz, 1, 1, device=self.gpu_id)
            # Generate fake image batch with G
            fake = self.generator(noise)
            label.fill_(0.)
            # Classify all fake batch with D
    ...
    def train(self, num_epochs):
        for epoch in range(num_epochs):
            g_loss, d_loss = self._run_epoch(epoch)
            if self.gpu_id == 0 and self.use_wandb:
                wandb.log({
                    "generator_loss": g_loss,
                    "discriminator_loss": d_loss
                })
            if self.gpu_id == 0 and epoch == num_epochs - 1:
